defmodule Couchie.Transcoder do
  ...
  def do_encode_value(flag, value) when (flag &&& @str_flag) === @str_flag and is_binary(value),
  ...
  def decode_value(flag, value) when (flag &&& @json_flag) === @json_flag,
    do: decode_value(flag ^^^ @json_flag, JSON.decode!(value, keys: :atoms))

  def decode_value(flag, value) when (flag &&& @raw_flag) === @raw_flag,
    do: decode_value(flag ^^^ @raw_flag, :erlang.binary_to_term(value))

  def decode_value(flag, value) when (flag &&& @str_flag) === @str_flag,
